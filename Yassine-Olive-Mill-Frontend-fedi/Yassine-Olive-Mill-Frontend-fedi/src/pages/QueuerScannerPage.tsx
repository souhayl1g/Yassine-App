...
export function QueuerScannerPage() {
  ...
  const checkPartiallyQueuedBatches = async () => {
    try {
      ...
      // Only consider it partially queued if there are remaining boxes > 0
      const hasActiveSession = data.hasPartiallyQueued && 
                              data.partiallyQueuedBatches.length > 0 && 
                              data.partiallyQueuedBatches[0].remainingBoxes > 0;
      
      setHasPartiallyQueued(hasActiveSession);
      if (hasActiveSession) {
  ...
  const handleQRResult = async (result: string) => {
    try {
      ...
      // Check if batch is currently being pressed using dedicated pressing-queue endpoint
      try {
        const statusRes = await api.get<any>(`/pressing-queue/batch/${ticketId}/pressing-status`);
        const statusData = getPayload<any>(statusRes);
        
        if (statusData.isCurrentlyPressed) {
          setIsCurrentlyPressed(true);
          setPressingRoomInfo(statusData.pressingRoomInfo);
        } else {
          setIsCurrentlyPressed(false);
          setPressingRoomInfo(null);
        }
      } catch (statusError) {
        console.error('Failed to check pressing status:', statusError);
        setIsCurrentlyPressed(false);
        setPressingRoomInfo(null);
      }
      ...
      // Check queue status using dedicated pressing-queue endpoint
      try {
        const queueRes = await api.get<any>('/pressing-queue/display-data');
        const queueData = getPayload<any>(queueRes);
        
        const queueItems = Array.isArray(queueData?.queueItems) ? queueData.queueItems : [];
  ...
  // Fetch ticket by code using dedicated pressing-queue endpoint
  const fetchTicketByCode = async (code: string | number): Promise<ScannedTicketData> => {
    ...
    try {
      const res = await api.get<any>(`/pressing-queue/batch/${idOrCode}/details`);
      const data = getPayload<any>(res);
      ...
      const errorMessage = e?.response?.status === 404 
        ? e?.response?.data?.error || 'التذكرة غير موجودة في النظام'
  ...
  const handleAddToQueue = async () => {
    ...
    // Check if batch is currently being pressed using dedicated pressing-queue endpoint
    try {
      const statusRes = await api.get<any>(`/pressing-queue/batch/${scannedTicket.id}/pressing-status`);
      const statusData = getPayload<any>(statusRes);
      ...
      if (statusData.isCurrentlyPressed) {
        toast({
          ...
          description: 'هذه التذكرة قيد العصر حالياً ولا يمكن إضافتها للطابور.',
        });
        return;
      }
    } catch (error) {
      console.error('Failed to check pressing status:', error);
    ...
    // Check for conflicting sessions before proceeding (only if there are remaining boxes > 0)
    if (hasPartiallyQueued && partiallyQueuedInfo && 
        partiallyQueuedInfo.remainingBoxes > 0 && 
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50 dark:from-gray-900 dark:to-gray-800 p-4">
        <div className="max-w-md mx-auto space-y-6">
          ...
          {/* Active Session Warning - only show if there are remaining boxes > 0 */}
              ...
              <div className="text-center">
                ...
                <div className="bg-white dark:bg-green-800/30 border border-green-300 dark:border-green-700 rounded-lg p-3 mb-3">
                  <p className="text-green-900 dark:text-green-100 font-bold text-xl mb-1">
                    تم تحميل جميع الصناديق
                  </p>
                  <p className="text-green-700 dark:text-green-300 text-sm">
                    لا توجد صناديق متبقية للإضافة
                  </p>
                </div>
                <p className="text-green-600 dark:text-green-400 text-sm">
                  هذه الدفعة موجودة بالفعل في قائمة الانتظار بالكامل
                </p>
              </div>
            </div>
          )}

          {/* Input Form - only show if there are boxes available */}
          {(scannedTicket.numberOfBoxes !== undefined && scannedTicket.numberOfBoxes > 0) && (
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg border space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  disabled={isCurrentlyPressed || (hasPartiallyQueued && partiallyQueuedInfo && partiallyQueuedInfo.remainingBoxes > 0 && partiallyQueuedInfo.id !== parseInt(scannedTicket.id)) || (currentBatchQueueInfo && currentBatchQueueInfo.totalBoxes > 0 && currentBatchQueueInfo.totalBoxes === currentBatchQueueInfo.boxesQueued)}
                />
              </div>
          ...
          <div className="flex gap-3">
            ...
            {/* Only show Add to Queue button if there are boxes available */}
            {(scannedTicket.numberOfBoxes !== undefined && scannedTicket.numberOfBoxes > 0) ? (
              <OliveButton
                onClick={handleAddToQueue}
                className="flex-1 text-lg py-3 bg-purple-600 hover:bg-purple-700"
                disabled={isSaving || isCurrentlyPressed || (hasPartiallyQueued && partiallyQueuedInfo && partiallyQueuedInfo.remainingBoxes > 0 && partiallyQueuedInfo.id !== parseInt(scannedTicket.id)) || (currentBatchQueueInfo && currentBatchQueueInfo.totalBoxes > 0 && currentBatchQueueInfo.totalBoxes === currentBatchQueueInfo.boxesQueued)}
              >
                {isSaving ? (
                  <>
                ...
                ) : (hasPartiallyQueued && partiallyQueuedInfo && partiallyQueuedInfo.remainingBoxes > 0 && partiallyQueuedInfo.id !== parseInt(scannedTicket.id)) ? (
                  <>
              ...
              <OliveButton
                disabled
                className="flex-1 text-lg py-3 bg-gray-400 cursor-not-allowed"
              >
                ✅ مكتملة التحميل
              </OliveButton>
            )}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50 dark:from-gray-900 dark:to-gray-800">
      <div className="flex flex-col h-screen">
        ...
        {/* Partially Queued Warning Banner - only show if there are remaining boxes > 0 */}
        {hasPartiallyQueued && partiallyQueuedInfo && partiallyQueuedInfo.remainingBoxes > 0 && (
